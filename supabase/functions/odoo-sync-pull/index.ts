...
import { OdooClient, parseMany2One } from '../_shared/odoo-client.ts';
import {
...
import { OdooProduct, OdooCategory, OdooPOSCategory, OdooTax, SyncPullResponse } from '../_shared/types.ts';
...
/**
 * Sincroniza categorías de producto desde Odoo
 */
...
/**
 * Sincroniza categorías de POS desde Odoo
 */
...
/**
 * Sincroniza productos desde Odoo (con paginación)
 */
async function syncProducts(odooClient: OdooClient, supabase: any): Promise<number> {
  ...
  while (true) {
    ...
    // Prevención de timeout: limitar a 5000 productos por sync
    if (offset > 5000) {
  ...
  if (allProducts.length > 0) {
    ...
    if (productsError) {
      throw new Error(`Error fetching inserted products: ${productsError.message}`);
    }
